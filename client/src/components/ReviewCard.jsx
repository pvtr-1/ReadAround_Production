...
const ReviewCard = ({ review }) => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await axios.get(
        `https://readaround.onrender.com:7000/api/user/details?id=${review.user_id}`
      );
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(
        `https://readaround.onrender.com:7000/api/review/${review.review_id}/comments`
      );
        ...
        if (!usersMap[comment.user_id]) {
          try {
            const userResponse = await axios.get(
              `https://readaround.onrender.com:7000/api/user/details?id=${comment.user_id}`
            );
